...
local function connection_handler(token, sock)
        ...
        return function(err, chunk)
                ...
                if not headers then
                        ...
                        if not (string.match(request, "^GET /" .. token .. " HTTP/1.1\r\n$")
                                ...
                                -- context: abort.
                                sock:close()
                                if config.server ~= "persistent" then
                                        close_server(server)
                                end
                                return
                        end
...
local function firenvim_start_server(token)
        ...
        local port = 0
        if config.globalSettings.server == 'persistent' and config.globalSettings.server_url ~= nil then
        ...
        server:listen(128, function(err)
                ...
                server:accept(sock)
                sock:read_start(connection_handler(token, sock))
        end)
        if port ~= 0 then
                return port
        end
